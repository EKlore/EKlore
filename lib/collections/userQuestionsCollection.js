...
Meteor.methods({
	...
	calcQuestionResult(data) {
		check(data, Object);
		check(data.userQuestionId, String);
		check(data.choiceSelected, String);
		let questionData = UserQuestions.findOne({ _id: userQuestionId });
		let ind = lodash.findIndex(questionData.choices, ['choiceId', data.choiceSelected]);
		return;
	}
});
